...
- **设备接入**：设备注册与 apiKey 鉴权，统一上报入口
- **批量上报**：单次最多 200 条，支持设备断点续传与重试
- **限流保护**：单设备令牌桶（Redis + Lua），容忍短时突发、限制长期速率
- **幂等去重**：Redis SETNX 前置去重 + 数据库唯一索引兜底
- **削峰落库**：RabbitMQ 异步消费 + 批量 `INSERT IGNORE`
- **最新状态**：Redis Cache-Aside，未命中自动回源并回填
- **查询统计**：历史轨迹分页查询，上报量按小时 / 天聚合
- **故障兜底**：消费失败重试 3 次后进入死信队列
...
> 说明：以上为本地开发环境实测数据，非生产环境压测结果；如需更正式的指标，建议在更高配置机器上用 JMeter 复测。
